...
def train_step(model, data, kg, rules, rules_mask, IM, Ht, optimizer, scheduler):
    """
    ...
    rules_mask = rules_mask.to(device)
    IM = IM.to(device)
    # randomly remove the training edges
    if cfg.rm_rate > 0:
        kg.remove_edges(rm_edges)
    score = model(src, rel, kg, rules, rules_mask, IM, Ht)
...
def evaluate(model, kg, rules, rules_mask, IM, Ht, set_flag, record=False) -> dict:
    """
    ...
    with torch.no_grad():
        ...
        ent_emb, rel_emb = model.aggragate_emb(kg, rules.to(device), rules_mask.to(device), IM.to(device), Ht)
